...
class StoreReports extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      regionNameShow: false,
      zoneNameShow: false,
      campaignStatusShow: false,
      campAssignToShow: false,
      departmentName: [],
      functionName: [],
      priorityName: [],
      ...
      regionNameOvrlayShow: false,
      zoneNameOvrlayShow: false,
      campaignStatusOvrlayShow: false,
      campAssignToOvrlayShow: false,
      indiDepartment: "",
      indiFunction: "",
      indiPriority: "",
      indiClaimStatus: "",
  ...
  handleClearTabData() {
    setTimeout(() => {
      ...
      this.selectNoRegionName();
      this.selectNoZoneName();
      this.selectNoCampaignStatus();
      this.selectNoCampAssignTo();
    }, 1);
  ...
  handleReginNameButton = () => {
    let regionShowOriginal = this.state.regionNameShow;
    let regionNameShow = !regionShowOriginal;
    let slaOvrlayShowOriginal = this.state.regionNameOvrlayShow;
    let regionNameOvrlayShow = !slaOvrlayShowOriginal;

    this.setState({
      regionNameShow,
      regionNameOvrlayShow,
    });
  };
  handleZoneNameButton = () => {
    let regionShowOriginal = this.state.zoneNameShow;
    let zoneNameShow = !regionShowOriginal;
    let slaOvrlayShowOriginal = this.state.zoneNameOvrlayShow;
    let zoneNameOvrlayShow = !slaOvrlayShowOriginal;
  ...
  handleCampaignAssignToButton = () => {
    let regionShowOriginal = this.state.CamAssignToShow;
    let CamAssignToShow = !regionShowOriginal;
    let slaOvrlayShowOriginal = this.state.CamAssignToOvrlayShow;
    let CamAssignToOvrlayShow = !slaOvrlayShowOriginal;

    this.setState({
      CamAssignToShow,
      CamAssignToOvrlayShow,
    });
  };
  /// select all campaign
  ...
  /// select all region
  selectAllRegionName = async (event) => {
    var indiRegionName = "";
    var checkboxes = document.getElementsByName("allReginName");
    document.getElementById("ReginNameValue").textContent = "All Selected";
    for (var i in checkboxes) {
      if (checkboxes[i].checked === false) {
        checkboxes[i].checked = true;
      }
    }
    if (this.state.CampRegionData !== null) {
      this.state.CampRegionData.forEach(allReginId);
      function allReginId(item) {
        indiRegionName += item.regionID + ",";
      }
    }
    await this.setState({
      indiRegionName,
    });
    this.handleGetCampaignUser();
  };
  /// select all zone
  selectAllZoneName = async (event) => {
    var indiZoneName = "";
    var checkboxes = document.getElementsByName("allZoneName");
    document.getElementById("ZoneNameValue").textContent = "All Selected";
    for (var i in checkboxes) {
      if (checkboxes[i].checked === false) {
        checkboxes[i].checked = true;
      }
    }
    if (this.state.CampZoneData !== null) {
      this.state.CampZoneData.forEach(allReginId);
      function allReginId(item) {
        indiZoneName += item.value + ",";
      }
    }
    await this.setState({
      indiZoneName,
  ...
  /// handle no select Region
  selectNoRegionName = async (event) => {
    var checkboxes = document.getElementsByName("allReginName");
    document.getElementById("ReginNameValue").textContent = "Select";
    for (var i in checkboxes) {
      if (checkboxes[i].checked === true) {
        checkboxes[i].checked = false;
      }
  ...
  /// handle no select Zone
  selectNoZoneName = async (event) => {
    var checkboxes = document.getElementsByName("allZoneName");
    document.getElementById("ZoneNameValue").textContent = "Select";
    for (var i in checkboxes) {
      if (checkboxes[i].checked === true) {
        checkboxes[i].checked = false;
      }
  ...
  /// hanlde select Individual region name
  selectIndividualRegionName = async (issueId, event) => {
    var indiRegionName = this.state.indiRegionName;
    var separator = ",";
    var values = indiRegionName.split(separator);
    if (event.target.checked) {
      var flag = values.includes(issueId.toString());
      if (!flag) {
        values.unshift(issueId);
        indiRegionName = values.join(separator);
      }
      await this.setState({
        indiRegionName,
      });
      document.getElementById("ReginNameValue").textContent =
        this.state.indiRegionName.split(",").length - 1 + " selected";
    } else {
      for (var i = 0; i < values.length; i++) {
        if (values[i] == issueId) {
          values.splice(i, 1);
      ...
      if (this.state.indiRegionName.split(",").length - 1 !== 0) {
        document.getElementById("ReginNameValue").textContent =
          this.state.indiRegionName.split(",").length - 1 + " selected";
      } else {
        document.getElementById("ReginNameValue").textContent = "Select";
  ...
  /// hanlde select Individual zone name
  selectIndividualZoneName = async (issueId, event) => {
    var indiZoneName = this.state.indiZoneName;
    var separator = ",";
    var values = indiZoneName.split(separator);
    if (event.target.checked) {
      var flag = values.includes(issueId.toString());
      if (!flag) {
        values.unshift(issueId);
        indiZoneName = values.join(separator);
      }
      await this.setState({
        indiZoneName,
      });
      document.getElementById("ZoneNameValue").textContent =
        this.state.indiZoneName.split(",").length - 1 + " selected";
    } else {
      for (var i = 0; i < values.length; i++) {
        if (values[i] == issueId) {
          values.splice(i, 1);
      ...
      if (this.state.indiZoneName.split(",").length - 1 !== 0) {
        document.getElementById("ZoneNameValue").textContent =
          this.state.indiZoneName.split(",").length - 1 + " selected";
      } else {
        document.getElementById("ZoneNameValue").textContent = "Select";
  ...
  handleCampAssignToButton = () => {
    let slaShowOriginal = this.state.campAssignToShow;
    let campAssignToShow = !slaShowOriginal;
    let slaOvrlayShowOriginal = this.state.campAssignToOvrlayShow;
    let campAssignToOvrlayShow = !slaOvrlayShowOriginal;
    this.setState({
      campAssignToShow,
      campAssignToOvrlayShow,
    });
  };
  ...
  selectAllCampAssignTo = async (event) => {
    var indiCampAssignTo = "";
    var checkboxes = document.getElementsByName("allCampAssignTo");
    document.getElementById("campaignAssignedToValue").textContent =
      "All Selected";
    for (var i in checkboxes) {
      if (checkboxes[i].checked === false) {
        checkboxes[i].checked = true;
    ...
    if (this.state.CampaignUserData !== null) {
      this.state.CampaignUserData.forEach(allCampaignId);
      function allCampaignId(item) {
        indiCampAssignTo += item.userID + ",";
      }
    }
    await this.setState({
      indiCampAssignTo,
  ...
  selectNoCampAssignTo = async (event) => {
    var checkboxes = document.getElementsByName("allCampAssignTo");
    document.getElementById("campaignAssignedToValue").textContent = "Select";
    for (var i in checkboxes) {
      if (checkboxes[i].checked === true) {
        checkboxes[i].checked = false;
      }
  ...
  selectIndividualCampAssignTo = async (issueId, event) => {
    var indiCampAssignTo = this.state.indiCampAssignTo;
    var separator = ",";
    var values = indiCampAssignTo.split(separator);
    if (event.target.checked) {
      var flag = values.includes(issueId.toString());
      if (!flag) {
        values.unshift(issueId);
        indiCampAssignTo = values.join(separator);
      }
      await this.setState({
        indiCampAssignTo,
      });
      document.getElementById("campaignAssignedToValue").textContent =
        this.state.indiCampAssignTo.split(",").length - 1 + " selected";
    } else {
      for (var i = 0; i < values.length; i++) {
        if (values[i] == issueId) {
          values.splice(i, 1);
      ...
      if (this.state.indiCampAssignTo.split(",").length - 1 !== 0) {
        document.getElementById("campaignAssignedToValue").textContent =
          this.state.indiCampAssignTo.split(",").length - 1 + " selected";
      } else {
        document.getElementById("campaignAssignedToValue").textContent =
  ...
  handleGetCampaignUser() {
    let self = this;
    axios({
      method: "post",
      url: config.apiUrl + "/StoreUser/GetStoreReportUser",
      params: {
        RegionID: this.state.indiRegionName.substring(
          ",",
          this.state.indiRegionName.length - 1
        ),
        ZoneID: this.state.indiZoneName.substring(
          ",",
          this.state.indiZoneName.length - 1
        ),
      },
      headers: authHeader(),
    })
  ...
  handleGetStoreReportSearch(activeTabId) {
    ...
    if (activeTabId === 3) {
      paramData = {
        ...
        CampaignAssignedIds: this.state.indiCampAssignTo,
        CampaignStartDate:
        ...
        CampaignRegion: this.state.indiRegionName,
        CampaignZone: this.state.indiZoneName,
      };
    }
    if (activeTabId === 4) {
      paramData = {
  ...
  handleEditReport = async (rowData) => {
    ...
    await setTimeout(async () => {
      ...
      this.setState({
        ...
        indiRegionName: allTab["CampaignRegion"],
        indiZoneName: allTab["CampaignZone"],
        indiCampaignStatus: allTab["CampaignStatusids"],
        indiCampAssignTo: allTab["CampaignAssignedIds"],
  ...
  handleDownload = (id) => {
    axios({
      ...
      params: {
        ReportID: id,
        RegionID: this.state.indiRegionName,
        ZoneID: this.state.indiZoneName,
      },
    })
      .then(function(res) {
  ...
  handleGetRegion() {
    ...
    axios({
      ...
      .then(function(response) {
        ...
        if (message === "Success") {
          self.setState({
            CampRegionData: data.regionMasters,
            CampZoneData: data.enumModels,
            // campaignRegion: data[0].regionID,
            // campaignZone: data[0].zoneID,
          });
          // self.handleGetCampaignUser(data[0].regionID, data[0].zoneID);
        }
      })
      .catch((response) => {
        console.log(response);
      ...
      <Fragment>
        ...
        <div className="container-fluid setting-title setting-breadcrumb">
          ...
          >
            ...
            <div
              id="overlayReginName"
              className={this.state.regionNameOvrlayShow ? "show" : ""}
              onClick={this.handleReginNameButton}
            />
            <div
              id="overlayZoneName"
              className={this.state.zoneNameOvrlayShow ? "show" : ""}
              onClick={this.handleZoneNameButton}
            />
            <div
              id="overlayCampaignAssignTo"
              className={this.state.campAssignToOvrlayShow ? "show" : ""}
              onClick={this.handleCampAssignToButton}
            />
            <div
                                ...
                                {this.state.functionName !== null &&
                                  this.state.functionName.map((item, i) => (
                                    <li key={i}>
                                      ...
                                      />
                                      <label
                                        htmlFor={"f" + item.functionID}
                                        title={item.funcationName}
                                      >
                                        {item.funcationName.length > 20
                                          ? item.funcationName
                                              .substr(0, 20)
                                              .concat("...")
                                          : item.funcationName}
                                ...
                                {this.state.priorityName !== null &&
                                  this.state.priorityName.map((item, i) => (
                                    <li key={i}>
                                      ...
                                      />
                                      <label
                                        htmlFor={"p" + item.priorityID}
                                        title={item.priortyName}
                                      >
                                        {item.priortyName.length > 20
                                          ? item.priortyName
                                              .substr(0, 20)
                                              .concat("...")
                                          : item.priortyName}
                          ...
                          <div className="cat-mainbox">
                            <div className="category-button">
                              <ul>
                                <li>
                                  <label
                                    onClick={this.selectAllRegionName.bind(
                                      this
                                    )}
                                  >
                                    Select All
                                ...
                                <li>
                                  <label
                                    onClick={this.selectNoRegionName.bind(this)}
                                  >
                                    Clear
                                  </label>
                                </li>
                            ...
                            <div className="category-box category-scroll">
                              <ul className="m-0">
                                {this.state.CampRegionData !== null &&
                                  this.state.CampRegionData.map((item, i) => (
                                    <li key={i}>
                                      <input
                                        type="checkbox"
                                        id={"region" + item.regionID}
                                        name="allReginName"
                                        onChange={this.selectIndividualRegionName.bind(
                                          this,
                                          item.regionID
                                        )}
                                        checked={
                                          this.state.indiRegionName !==
                                          undefined
                                            ? this.state.indiRegionName
                                                .split(",")
                                                .find(
                                      ...
                                      />
                                      <label
                                        htmlFor={"region" + item.regionID}
                                        title={item.regionName}
                                      >
                                        {item.regionName.length > 20
                                          ? item.regionName
                                              .substr(0, 20)
                                              .concat("...")
                                          : item.regionName}
                          ...
                          <div className="cat-mainbox">
                            <div className="category-button">
                              <ul>
                                <li>
                                  <label
                                    onClick={this.selectAllZoneName.bind(this)}
                                  >
                                    Select All
                                  </label>
                                </li>
                                <li>
                                  <label
                                    onClick={this.selectNoZoneName.bind(this)}
                                  >
                                    Clear
                                  </label>
                                </li>
                            ...
                            <div className="category-box category-scroll">
                              <ul className="m-0">
                                {this.state.CampZoneData !== null &&
                                  this.state.CampZoneData.map((item, i) => (
                                    <li key={i}>
                                      <input
                                        type="checkbox"
                                        id={"zone" + item.value}
                                        name="allZoneName"
                                        onChange={this.selectIndividualZoneName.bind(
                                          this,
                                          item.value
                                        )}
                                        checked={
                                          this.state.indiZoneName !== undefined
                                            ? this.state.indiZoneName
                                                .split(",")
                                                .find(
                                                  (num) =>
                          ...
                          <div className="cat-mainbox">
                            <div className="category-button">
                              <ul>
                                <li>
                                  <label
                                    onClick={this.selectAllCampAssignTo.bind(
                                      this
                                    )}
                                  >
                                    Select All
                                ...
                                <li>
                                  <label
                                    onClick={this.selectNoCampAssignTo.bind(
                                      this
                                    )}
                                  >
                                    Clear
                                ...
                                {this.state.CampaignUserData !== null &&
                                  this.state.CampaignUserData.map((item, i) => (
                                    <li key={i}>
                                      <input
                                        type="checkbox"
                                        id={"cmpAssnTo" + item.userID}
                                        name="allCampAssignTo"
                                        onChange={this.selectIndividualCampAssignTo.bind(
                                          this,
                                          item.userID
                                        )}
                                        checked={
                                          this.state.indiCampAssignTo !==
                                          undefined
                                            ? this.state.indiCampAssignTo
                                                .split(",")
                                                .find(
