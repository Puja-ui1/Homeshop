...
class TicketCRMRole extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      //   { moduleId: 12, moduleName: "Call Logs", isActive: false },
      //   { moduleId: 13, moduleName: "Edit Call Access", isActive: false },
      // ],
      modulesList: [],
      updateRoleName: "",
      updateRoleisActive: "",
  ...
  handleModuleListArray = () => {
    let self = this
    axios({
      method: "post",
      url: config.apiUrl + "/Module/GetCRMModuleMaster",
      headers: authHeader(),
    })
      .then(function (res) {
        if (res.data.statusCode === 200) {
          self.setState({
            modulesList: res.data.responseData,
          })
        }
      })
  ...
  handleModulesDefault = async () => {
    ...
    for (let i = 0; i < modulesList.length; i++) {
      if (modulesList[i].modulestatus === true) {
        ModulesEnabled += modulesList[i].moduleId + ",";
      } else if (modulesList[i].modulestatus === false) {
  ...
  checkModule = async (moduleId) => {
    ...
    for (let i = 0; i < modulesList.length; i++) {
      if (modulesList[i].moduleID === moduleId) {
        isActive = modulesList[i].modulestatus;
        modulesList[i].modulestatus = !isActive;
      }
    }
    for (let i = 0; i < modulesList.length; i++) {
      if (modulesList[i].modulestatus === true) {
              ...
              <div className="col-md-4">
                <div className="store-col-2">
                  <div className="createSpace">
                    ...
                    <div className="moduleListClass mt-2 mb-2">
