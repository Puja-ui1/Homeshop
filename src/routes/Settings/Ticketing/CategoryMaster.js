...
class CategoryMaster extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      ListOfIssue: "",
      ShowSubCate: false,
      ShowSubSubCate: false,
      editShowSubCate: false,
      editShowSubSubCate: false,
      ShowIssuetype: false,
    ...
    this.handleAddSubSubCategory = this.handleAddSubSubCategory.bind(this);
  ...
  StatusOpenModel(data, header) {
    if (
      ...
      this.state.sortFilterSubSubCategory.length === 0 ||
  ...
  StatusCloseModel() {
    if (this.state.tempcategoryGridData.length > 0) {
      this.setState({
        ...
        sortFilterSubSubCategory: this.state.sortSubSubCategory,
      ...
      if (this.state.sortColumn === "subsubCategoryName") {
        if (this.state.ssubCategoryNameFilterCheckbox === "") {
        } else {
          this.setState({
            sbrandNameFilterCheckbox: "",
  ...
  handleGetSubSubCategoryList = async (id) => {
    ...
    var SubCategory_Id = "";
    if (id === "edit") {
      var data = this.state.SubCategoryDropData.filter(
        (x) => x.categoryName === this.state.editCategory.categoryName
      );
      SubCategory_Id = data[0].subCategoryID;
    } else {
      var data = this.state.SubCategoryDropData.filter(
        (x) => x.categoryName === this.state.list1Value
      );
    ...
    await axios({
      ...
      .then(function (res) {
        let data = res.data.responseData;
        self.setState({ SubSubCategoryDropData: data });
      })
      .catch((data) => {
        console.log(data);
      });
  ...
  //subsub
  handleAddSubSubCategory(value, check) {
    const TranslationContext = this.state.translateLanguage.default;
    let self = this;
    var finalId = 0;
    if (check === "edit") {
    ...
    if (
      ...
      axios({
        ...
        data: {
          ...
          Status: activeStatus == false ? false : true,
        },
      })
        .then(function (res) {
          let status = res.data.message;
  ...
  handleSubSubCatOnChange = (value) => {
    ...
    if (value !== NEW_ITEM) {
      ...
      setTimeout(() => {
        if (this.state.ListOfSubSubCate) {
          this.handleGetIssueTypeList();
        }
      }, 1);
    } else {
      this.setState({ ShowSubSubCate: true });
    }
  };
  ...
  handleSearchEditSubSubCategoryData(data) {
    this.handleSearchSubSubCategoryData(data, "edit");
  }
  ///handle Search Subsub category data
  handleSearchSubSubCategoryData(data, check) {
    const TranslationContext = this.state.translateLanguage.default;
    let self = this;
    if (this.state.categoryDropData.length > 0) {
      if (data.length > 2) {
                  ...
                  <div className="createSpace cus-cs">
                    ...
                    {/* <div className="divSpace">
                      <div className="dropDrownSpace">
                        ...
                        <Select
                          showSearch={true}
                          value={this.state.ListOfSubSubCate}
                          style={{ width: "100%" }}
                          onChange={this.handleSubSubCatOnChange}
                          onSearch={this.handleSearchSubSubCategoryData.bind(this)}
                          notFoundContent="No Data Found"
                        >

                          {this.state.SubSubCategoryDropData !== null &&
                            this.state.SubSubCategoryDropData.map((item, o) => (
                              <Option key={o} value={item.subSubCategoryName}>
                        ...
                        {this.state.showMsgSubCategory === true ? (
                          <span
                            className="sweetAlert-inCategory"
                            style={{ marginTop: "-68px" }}
                            onClick={this.handleToggleSubSubcategoryAdd.bind(this)}
                          >
                            +
                            {TranslationContext !== undefined
                              ? TranslationContext.span.addnew
                        ...
                        <SweetAlert
                          show={this.state.ShowSubSubCate}
                          style={{ width: "320px" }}
                          title="Add New Sub Sub Category"
                          text="Enter new Sub Sub Category"
                          showCancelButton
                          type="input"
                          inputPlaceholder="Enter Sub Sub Category Name"
                          animation="slide-from-top"
                          validationMsg="Please enter a Sub Sub category!"
                          onConfirm={(inputValue) => {
                            inputValue = inputValue.trim();
                            if (inputValue !== "") {
                              this.setState({
                                ShowSSububCate: false,
                                ListOfSubSubCate: inputValue,
                              });
                              this.handleAddSubSubCategory(inputValue);
                            } else {
                              this.setState({
                                ShowSubSubCate: false,
                                ListOfSubSubCate: inputValue,
                          ...
                          onCancel={() => {
                            this.setState({ ShowSubSubCate: false });
                          }}
                          onEscapeKey={() =>
                            this.setState({ ShowSubSubCate: false })
                          }
