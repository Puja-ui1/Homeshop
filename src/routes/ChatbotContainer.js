...
const ChatbotContainer = () => {
    ...
    const handelNewChat = (chat = {}) => {
        ...
        chatService.PostWithParams("/CustomerChat/UpdateCustomerChatStatus", inputParams)
            .then((response) => {
                ...
                if (message === "Success" && responseData) {
                    ...
                    // Update ongoingChatsData and remove the chat from newChatsData
                    //setongoingChatsData(prev => [...prev, chat]);
                    setongoingChatsData((prev) => [chat, ...prev]);
                    setnewChatsData(prev => prev.filter(c => c.chatID !== chat.chatID));

                    // Update local storage
                    localStorage.setItem('ongoingChatsData', JSON.stringify([...ongoingChatsData, chat]));
                    localStorage.setItem('newChatsData', JSON.stringify(newChatsData.filter(c => c.chatID !== chat.chatID)));
                }
                var objNewChat = {
    ...
    console.log(messageData,"Inside ChatbotContainer");
        ...
        <div className="container-fluid ">
            <div className="row ">
                <div className="col-lg-3 p-0 ">
                    <div className="chatSideBox_wrap">
                        <div className="ongoing_wrap">
                            <p className="Heading">Ongoing Chats ({ongoingChatsData?.length < 9 ? "0" + ongoingChatsData?.length : ongoingChatsData?.length})</p>
                        ...
                        <div className="newchat_wrap">
                            <p className="Heading">New Chats ({newChatsData?.length < 9 ? "0" + newChatsData?.length : newChatsData?.length})</p>
                            <ul className="userchatlist_wrap ">
                ...
                <div className="col-lg-9 p-0">
                    {isHistoricalChat === 'history' ? <HistoricalChat UserName={UserName} /> : 
                        <ChatBox
                            isHistoricalChat={isHistoricalChat}
                            setIsHistoricalChat={setIsHistoricalChat}
                            ...
                            setchatId={setchatId}
                            handleHistoricalChat ={handleHistoricalChat}
                            handelNewChat={handelNewChat}
                            setongoingChatsData= {setongoingChatsData}
                            ongoingChatsData= {ongoingChatsData}
                        />}
                </div>
            </div>
        </div>
