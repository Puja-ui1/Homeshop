...
class UserProfile extends Component {
  ...
  fileUpload(e) {
    ...
    if(selectedFiles.length > 0 && selectedFiles[0].name.match(/\.(jpg|jpeg|png|gif)$/i)){
      allFiles.push(selectedFiles[0]);
    this.setState({
      fileName: allFiles,
    });
    }else{
        NotificationManager.warning("Please choose a valid image file");
    }
    
    
  }
  fileDrop = (e) => {
    if(e.dataTransfer.files[0].name.match(/\.(jpg|jpeg|png|gif)$/i)){
